...
int WrkApi::Recv(std::string& message, std::string& addr)
{
    int rc = 0;
...
int WrkApi::Send(std::string& msg, std::string& to)
{
    MDP_ASSERT(!to.empty());

    mdp::MdpMsg mdp_message;
    mdp_message.PushFront(msg);
    mdp_message.PushFront("");
    mdp_message.PushFront(to);

    return RealSend(MDPW_REPORT, "", mdp_message);
}
...
int WrkApi::ProcessRecvCmd(mdp::MdpMsg& mdp_message, std::string& message, std::string& from)
{
    ...
    if (cmd == std::string(MDPW_REQUEST))
    {
        MDP_ASSERT(mdp_message.Size() == 3);
